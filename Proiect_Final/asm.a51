org 0000h
    ...
    sjmp $ ; Infinite loop to prevent re-running the setup
...
LCD:
    ...
    mov a, #90h ; Pozitionare cursor pe linia a doua la pozitia 0
    acall instructiuni
    acall delay_timer
    ...
    mov a, #0D0h ; Pozitionare cursor pe linia a doua la pozitia 0
    acall instructiuni
    acall delay_timer
...
display_humidity:
    ; Calculeaza umiditatea in functie de temperatura actuala
    mov a, P1 ; Citeste temperatura actuala
    mov b, #0Ah
    subb a, b
    jc calc_high_humidity
    
    mov a, P1
    subb a, #0Ah
    add a, #32h
    sjmp finish_humidity
...
finish_humidity:
    ; Converteste valoarea umiditatii in zeci si unitati
    mov b, #0Ah
    div ab
    mov r6, a ; Salveaza zecile
    mov r7, b ; Salveaza unitatile

    ; Pozitionare cursor pe a treia linie la pozitia 13
    mov a, #9Dh
    acall instructiuni
    acall delay_timer
...
HEART:
    ...
    MOV A, #0DDH ; Set cursor position for heart character (C8H)
    ACALL instructiuni
    MOV A, #00H ; Display heart character
    ACALL date
    ACALL delay_timer
...
CARACTER:
    ...
    MOV A, #0DEH ; Set cursor position for custom character (CAH)
    ACALL instructiuni
    MOV A, #01H ; Display custom character
    ACALL date
    ACALL delay_timer
...
OMULET:
    ...
    MOV A, #0DFH ; Set cursor position for custom character (CAH)
    ACALL instructiuni
    MOV A, #02H ; Display custom character
    ACALL date
    ACALL delay_timer
    RET
; Control releu
